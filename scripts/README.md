# SolMobile Runloop (Scripts + Cursor commands) v1.1

This bundle makes your PR packet executable in the **SolMobile** repo.
...
- Deterministic scripts:
  ...
  - `scripts/bootstrap.sh` (sanity checks for Xcode tooling)
- Cursor commands:
  - `/build-pr`, `/verify-pr`, `/run-pr`, `/pr-body-hygiene`
- Cursor rules:
  - `agentpacket.mdc`, `pr-body-hygiene.mdc`
...
## How to run
From terminal (Codex.app, Cursor terminal, etc):
```sh
...
- `SOLMOBILE_PR` (optional): PR URL or number for gh pr edit in pr_body_hygiene
- `REPO_SLUG` (optional): owner/repo for cross-repo gh edits
- `XCODE_DEVELOPER_DIR` (optional): set to pin a specific Xcode (exported as `DEVELOPER_DIR`)

## Xcode note
SolMobile gates typically require:
- macOS
- Xcode / Command Line Tools
- simulator availability (depending on your `GATE_UNIT` command)

This pack does **not** guess your scheme/workspace.
Put the exact `xcodebuild ...` commands in `AGENTPACK.md` under Gates.
